...
class CrossProductViewHelper extends AbstractSubTemplateRenderingViewHelper {
	...
	public function render($values, $matrixMode = NULL) {
		...
		$expected = NULL;
		if ($this->viewHelperVariableContainer->exists('Flowpack\Viewhelpertest\ViewHelpers\ExpectedViewHelper', 'source')) {
		...
		for ($i = 1; $i <= count($variableNames); $i++) {
			...
			for ($j = 1; $j <= count($variableNames); $j++) {
				...
				if ($this->viewHelperVariableContainer->exists('Flowpack\Viewhelpertest\ViewHelpers\HighlightViewHelper', 'results')) {
					$this->viewHelperVariableContainer->remove('Flowpack\Viewhelpertest\ViewHelpers\HighlightViewHelper', 'results');
				}
